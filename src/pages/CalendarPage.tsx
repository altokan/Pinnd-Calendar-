...
import { 
  ...
  Briefcase, HeartPulse, Plane, PartyPopper, Dumbbell, Star, Calendar as CalendarIcon
} from 'lucide-react';
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-stone-900/40 backdrop-blur-md">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-white rounded-[3.5rem] w-full max-w-xl p-10 shadow-3xl relative border-4 border-white">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <input required value={title} onChange={e => setTitle(e.target.value)} placeholder="What's happening?" className="w-full text-2xl font-serif italic border-none focus:ring-0 p-0 text-stone-900 outline-none" />
                ...
                {/* خريطة صغيرة للمعاينة إذا وجد عنوان */}
                {location && (
                  <div className="h-32 w-full rounded-2xl overflow-hidden border-2 border-stone-50 grayscale hover:grayscale-0 transition-all">
                    <iframe width="100%" height="100%" frameBorder="0" src={`https://www.google.com/maps?q=${encodeURIComponent(location)}&output=embed`} />
                  </div>
                )}
                ...
                <div className="flex gap-4 items-center">
                   <label className="flex-1 flex items-center justify-center gap-3 p-4 bg-stone-50 rounded-2xl cursor-pointer border-2 border-dashed border-stone-200">
                      ...
                      {previewUrl ? <img src={previewUrl} className="w-8 h-8 object-cover rounded-lg" /> : <ImageIcon size={20} className="text-stone-300" />}
                      <span className="text-[10px] font-black text-stone-500 uppercase">6MB MAX</span>
                   </label>
