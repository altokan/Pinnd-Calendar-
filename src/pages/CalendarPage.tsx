...
import { 
  ...
  Briefcase, HeartPulse, Plane, PartyPopper, Dumbbell, Star, FileText, Bell, BellRing, Eye, CalendarOff
} from 'lucide-react';
...
export default function CalendarPage() {
  ...
  const [isPreviewOpen, setIsPreviewOpen] = useState(false); // View Modal
  const [isTimelineOpen, setIsTimelineOpen] = useState(false);
  const [selectedPin, setSelectedPin] = useState<any>(null); // Pin for viewing
  ...
  const openEditModal = (pin: any) => {
    ...
    setIsPreviewOpen(false); // إغلاق المعاينة لفتح التعديل
    setIsModalOpen(true);
  };
  ...
  const resetForm = () => {
    ...
    setPreviewUrl(null); setIsModalOpen(false); setLoading(false);
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-8 pb-32 font-sans">
      {/* Calendar Header - تبقى كما هي لجمالها */}
              ...
              <div key={idx} onClick={() => { setSelectedDate(day); setIsTimelineOpen(true); }}
                ...
                <div className="flex flex-col gap-1.5">
                  {dayPins.slice(0, 2).map((pin) => (
                    <div key={pin.id} className="flex items-center gap-2 bg-white/80 p-1.5 rounded-lg border border-stone-50 shadow-sm overflow-hidden animate-in fade-in">
      ...
      <AnimatePresence>
        {isTimelineOpen && (
          <>
            ...
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-2xl rounded-t-[4rem] shadow-2xl z-50 max-h-[85vh] overflow-y-auto border-t-4 border-white">
              ...
              <div className="max-w-3xl mx-auto px-8 pb-20">
                <div className="flex items-center justify-between mb-10 sticky top-0 bg-white/10 py-6 z-10 backdrop-blur-sm">
                  ...
                  <button onClick={() => setIsModalOpen(true)} className="p-5 bg-stone-900 text-white rounded-[2rem] shadow-xl hover:scale-105 active:scale-95 transition-transform"><Plus size={28} /></button>
                </div>
                
                <div className="space-y-4">
                  {selectedDayPins.length > 0 ? selectedDayPins.map((pin) => (
                    <div key={pin.id} onClick={() => openPreview(pin)} className="flex items-center gap-6 p-6 bg-white rounded-[2.5rem] border-2 border-stone-50 shadow-sm hover:border-stone-200 cursor-pointer transition-all group">
                      ...
                      <div className="flex-1">
                        ...
                        {pin.location && <p className="text-[10px] font-bold text-stone-400 mt-1 uppercase tracking-widest line-clamp-1">{pin.location}</p>}
                      </div>
                      {pin.imageUrl && <img src={pin.imageUrl} className="w-14 h-14 object-cover rounded-2xl shadow-sm" />}
                      <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                        <Eye size={18} className="text-stone-300" />
                      </div>
                    </div>
                  )) : (
                    /* --- الحالة الفارغة المطلوبة --- */
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-24 text-center flex flex-col items-center gap-4">
                        <div className="w-20 h-20 bg-stone-50 rounded-full flex items-center justify-center">
                            <CalendarOff size={32} className="text-stone-200" />
                        </div>
                        <p className="text-2xl font-serif italic text-stone-300">No events scheduled for today...</p>
                        <button onClick={() => setIsModalOpen(true)} className="text-[10px] font-black uppercase tracking-[0.2em] text-stone-400 hover:text-stone-900 transition-colors">Create something new</button>
                    </motion.div>
                  )}
                </div>
              </div>
      ...
      {/* View Detail Modal - نافذة العرض المنبثقة */}
      <AnimatePresence>
        {isPreviewOpen && selectedPin && (
          <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-lg">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-[3.5rem] w-full max-w-2xl overflow-hidden shadow-3xl border-4 border-white relative">
              <button onClick={() => setIsPreviewOpen(false)} className="absolute top-6 right-6 p-3 bg-white/80 backdrop-blur-md rounded-full shadow-lg z-10 text-stone-900"><X size={20}/></button>
              
              {selectedPin.imageUrl && (
                <img src={selectedPin.imageUrl} className="w-full h-64 object-cover border-b-4 border-white" />
              )}
              ...
              <div className="p-10">
                <div className="flex justify-between items-start mb-6">
                   <div>
                     <div className="flex items-center gap-2 mb-2">
                        <div className={cn("w-3 h-3 rounded-full", CATEGORIES.find(c => c.id === selectedPin.category)?.color)} />
                        <span className="text-[10px] font-black uppercase tracking-widest text-stone-400">{selectedPin.category}</span>
                     </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  <div className="flex items-center gap-4 bg-stone-50 p-5 rounded-3xl">
                    <Clock className="text-stone-300" size={24} />
                    <div>
                      <p className="text-[9px] font-black text-stone-400 uppercase tracking-tighter">Time</p>
                      <p className="text-lg font-bold text-stone-900">{selectedPin.time}</p>
                    </div>
                  </div>
                  {selectedPin.reminder !== 'none' && (
                    <div className="flex items-center gap-4 bg-amber-50 p-5 rounded-3xl">
                      <BellRing className="text-amber-500" size={24} />
                      <div>
                        <p className="text-[9px] font-black text-amber-400 uppercase tracking-tighter">Reminder</p>
                        <p className="text-lg font-bold text-amber-600 capitalize">{selectedPin.reminder}</p>
                      </div>
                    </div>
                  )}
                </div>
                ...
                {selectedPin.notes && (
                  <div className="mb-8 p-6 bg-stone-50/50 rounded-3xl border border-stone-100">
                    <p className="text-[10px] font-black text-stone-400 uppercase tracking-widest mb-3 flex items-center gap-2"><FileText size={14}/> Notes</p>
                    <p className="text-stone-600 leading-relaxed font-medium">{selectedPin.notes}</p>
                  </div>
                )}

                {selectedPin.location && (
                  <div className="space-y-4">
                    <p className="text-[10px] font-black text-stone-400 uppercase tracking-widest flex items-center gap-2"><MapPin size={14}/> {selectedPin.location}</p>
                    <div className="h-40 w-full rounded-[2.5rem] overflow-hidden grayscale border-2 border-stone-50">
                       <iframe width="100%" height="100%" frameBorder="0" src={`https://maps.google.com/maps?q=${encodeURIComponent(selectedPin.location)}&t=&z=13&ie=UTF8&iwloc=&output=embed`} />
                    </div>
                  </div>
                )}
              </div>
      ...
      {/* Form Modal (Add/Edit) - تبقى كما هي مع تعديلات بسيطة */}
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-stone-900/40 backdrop-blur-md">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-white rounded-[3.5rem] w-full max-w-xl p-10 shadow-3xl relative border-4 border-white">
              ...
              <h3 className="text-2xl font-serif italic mb-8">{editingId ? 'Modify Event' : 'Create New'}</h3>
              <form onSubmit={handleSubmit} className="space-y-6">
                <input required value={title} onChange={e => setTitle(e.target.value)} placeholder="Event Title" className="w-full text-2xl font-serif italic border-none focus:ring-0 p-0 text-stone-900 outline-none placeholder:text-stone-200" />
                <div className="flex gap-3">
                  <div className="bg-stone-50 p-4 rounded-2xl flex-1 flex items-center gap-3"><Clock size={18} className="text-stone-300"/><input type="time" value={time} onChange={e => setTime(e.target.value)} className="bg-transparent border-none p-0 text-sm font-bold w-full outline-none" /></div>
                  <div className="bg-stone-50 p-4 rounded-2xl flex-[2] flex items-center gap-3"><MapPin size={18} className="text-stone-300"/><input value={location} onChange={e => setLocation(e.target.value)} placeholder="Location" className="bg-transparent border-none p-0 text-sm font-bold w-full outline-none" /></div>
                </div>
                <div className="bg-stone-50 p-4 rounded-2xl border border-stone-100"><textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Details..." className="bg-transparent border-none p-0 text-sm font-bold w-full outline-none h-16 resize-none" /></div>
                <div className="flex gap-4">
                  <label className="flex-1 flex items-center justify-center gap-3 p-4 bg-stone-50 rounded-2xl cursor-pointer border-2 border-dashed border-stone-200 transition-all hover:bg-stone-100">
                    <input type="file" onChange={e => { const file = e.target.files?.[0]; if(file) { setImageFile(file); setPreviewUrl(URL.createObjectURL(file)); } }} className="hidden" accept="image/*" />
                  ...
                  <button disabled={loading} type="submit" className="flex-[2] py-5 bg-stone-900 text-white rounded-[1.8rem] font-black shadow-xl flex items-center justify-center gap-3 active:scale-95 transition-all">
