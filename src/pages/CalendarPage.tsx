  ...
  return (
    <div className="min-h-screen bg-[#f8f5f2] p-6 pb-24 font-sans text-stone-800">
      ...
      <div className="max-w-4xl mx-auto">
        {viewMode === 'grid' ? (
          ...
          <div className="relative border-l-2 border-stone-200 ml-4 pl-8 space-y-8 py-4">
              ...
              return (
                <motion.div key={e.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="relative group">
                  ...
                  <div 
                    ...
                    className="bg-white p-6 rounded-[2rem] shadow-sm border border-stone-100 flex gap-6 items-center cursor-pointer hover:shadow-md transition-all"
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-stone-400 text-sm">{e.location}</p>
                    </div>
            ...
            {events.length === 0 && <p className="text-stone-400 font-bold italic">لا توجد أحداث مجدولة...</p>}
          </div>
        )}
      </div>
      ...
      <button 
        ...
        className="fixed bottom-8 right-8 w-16 h-16 bg-blue-600 text-white rounded-full shadow-2xl flex items-center justify-center z-[500] active:scale-90 transition-all"
      >
        <Plus size={32} />
      </button>
            ...
            <motion.div initial={{ y: 100 }} animate={{ y: 0 }} className="bg-white rounded-[2.5rem] w-full max-w-md p-8 overflow-y-auto max-h-[90vh] shadow-2xl">
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between p-2 bg-stone-50 rounded-2xl">
                  {EVENT_TYPES.map(t => (
                    <button key={t.id} onClick={() => setForm({...form, type: t.id})} className={cn("p-3 rounded-xl transition-all", form.type === t.id ? "bg-white shadow-sm text-blue-600" : "text-stone-400")}><t.icon size={20} /></button>
                  ))}
                </div>
