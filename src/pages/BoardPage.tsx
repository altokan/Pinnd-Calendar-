...
export default function BoardPage() {
  ...
  // إصلاح دالة الرفع التي تسببت في الخطأ
  ...
  return (
    <div className="fixed inset-0 overflow-hidden bg-[#bc8a5f]" style={{ backgroundImage: `url('https://www.transparenttextures.com/patterns/cork-board.png')` }}>
      ...
      {/* البنر السفلي المرفوع لجميع الشاشات */}
      ...
      {/* المودال الكامل بالخيارات المطلوبة */}
      <AnimatePresence>
        {showCalendarModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[1100] bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
            <motion.div initial={{ y: 50 }} animate={{ y: 0 }} className="bg-white rounded-[2.5rem] p-6 w-full max-w-lg overflow-y-auto max-h-[90vh] shadow-2xl">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold">Schedule Idea</h3>
              ...
              <div className="space-y-4">
                ...
                <div className="relative space-y-1">
                  ...
                  <input type="text" placeholder="Search address..." className="w-full p-4 pl-12 bg-stone-100 rounded-2xl outline-none text-sm" value={eventLocation} onChange={e => handleLocationInput(e.target.value)} />
                  {suggestions.length > 0 && (
                    <div className="absolute top-full left-0 right-0 bg-white shadow-2xl rounded-2xl z-50 overflow-hidden border">
                      {suggestions.map((s, i) => (
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-stone-400 uppercase ml-2">Notes</label>
                  <textarea placeholder="Write extra details..." className="w-full p-4 bg-stone-100 rounded-2xl h-24 text-sm resize-none outline-none" value={eventNote} onChange={e => setEventNote(e.target.value)} />
                </div>
