...
// أيقونات أنواع الأحداث كما في الصورة
const EVENT_TYPES = [
  { id: 'food', icon: Utensils },
  { id: 'music', icon: Music },
  { id: 'med', icon: Stethoscope },
...
export default function BoardPage() {
  ...
  // حالة مودال إضافة حدث (الواجهة المرفقة في صورتك)
  ...
  // إصلاح: إضافة العناصر مع التحقق من وجود المستند لمنع الاختفاء
  ...
  return (
    <div className="fixed inset-0 overflow-hidden bg-[#bc8a5f] touch-none">
      ...
      {/* مودال إضافة حدث (نفس تصميم الصورة المرفقة) */}
      <AnimatePresence>
        {showEventModal && (
          <div className="fixed inset-0 z-[2000] flex items-end sm:items-center justify-center p-0 sm:p-4">
            ...
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="relative w-full max-w-md bg-white rounded-t-[2.5rem] sm:rounded-[2.5rem] p-8 shadow-2xl">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-black text-stone-800">New Event</h2>
                <button onClick={() => setShowEventModal(false)} className="p-2 bg-stone-100 rounded-full"><X size={20}/></button>
              </div>

              <div className="space-y-4">
                <div>
                  <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest block mb-1">Event Name</label>
                  <input type="text" value={eventData.title} onChange={(e) => setEventData({...eventData, title: e.target.value})} className="w-full bg-stone-50 rounded-2xl p-4 text-stone-800 font-bold border-none" />
                </div>

                <div className="grid grid-cols-2 gap-3">
                   <div>
                    <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest block mb-1">Date</label>
                    <div className="w-full bg-stone-50 rounded-2xl p-4 text-stone-800 font-bold">{selectedDate}</div>
                  </div>
                  <div>
                    <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest block mb-1">Time</label>
                ...
                <div className="flex items-center justify-between p-4 bg-stone-50 rounded-2xl">
                  <div className="flex items-center gap-2 font-bold text-stone-600 text-xs">
                    <Bell size={16} className={eventData.alert ? "text-blue-500" : "text-stone-300"} /> Enable Alert
                  </div>
                ...
                <div className="relative">
                  <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest block mb-1">Location Search</label>
                  <div className="flex items-center bg-stone-50 rounded-2xl p-4 gap-2">
                    <MapPin size={16} className="text-stone-400" />
                    <input placeholder="Search for a place..." className="bg-transparent border-none w-full outline-none font-bold text-xs" />
                  </div>
                </div>
