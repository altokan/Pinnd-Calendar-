...
import {
  ...
  Settings, Bell, Eye, EyeOff, Search, Plus, Send, Trash
} from "lucide-react";
...
export default function AdminPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 p-4 md:p-8 font-sans text-stone-900" dir="ltr">
      
      {/* Header */}
      <div className="flex flex-col gap-1">
        <h1 className="text-3xl font-bold tracking-tight">Admin Dashboard</h1>
        <p className="text-stone-500">Manage members, messages, and platform configurations.</p>
      </div>

      {/* Tabs Navigation */}
      <div className="flex flex-wrap gap-2 bg-stone-100/60 p-1.5 rounded-[2rem] border border-stone-200 w-fit">
        {[
      ...
      <AnimatePresence mode="wait">
        ...
        >
              ...
              <div className="bg-white rounded-[2.5rem] border border-stone-200 shadow-sm p-8 space-y-8">
                <div className="flex flex-col gap-6">
                  ...
                  <form onSubmit={handleCreateUser} className="grid md:grid-cols-4 gap-4">
                    ...
                    <button disabled={creating} className="bg-black text-white rounded-2xl font-bold hover:bg-stone-800 transition-colors disabled:opacity-50">
                ...
                <div className="relative">
                  ...
                  <input placeholder="Search members by name or email..." value={search}
                    onChange={e => setSearch(e.target.value)}
                    className="w-full pl-14 pr-6 py-4 rounded-2xl bg-stone-100/50 border-none focus:ring-2 ring-stone-200 outline-none text-sm font-medium" />
                </div>
                  ...
                  {filteredUsers.map(u => (
                    <div key={u.uid} className="flex justify-between items-center py-5 group">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-stone-100 flex items-center justify-center font-bold text-stone-500 uppercase tracking-tighter">
                          {u.username?.[0]}
                        </div>
                        <div>
                          <p className="font-bold text-stone-800">{u.username}</p>
                          <p className="text-sm text-stone-400 tracking-tight">{u.email}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2">
                        <button className="p-2.5 rounded-xl hover:bg-stone-100 text-stone-400 hover:text-black transition-all"
                                ...
                                onClick={() => sendPasswordResetEmail(auth, u.email).then(() => toast.success("Reset link sent"))}>
                          <Key size={18} />
                        </button>
          ...
          {activeTab === "messages" && (
            <div className="space-y-6">
              <div className="flex justify-between items-center px-2">
                <h3 className="text-xl font-bold tracking-tight">Support Inbox</h3>
                <button onClick={() => { if(confirm("Clear all messages?")) { /* delete all logic */ } }} 
                  className="text-sm font-bold text-rose-600 hover:bg-rose-50 px-4 py-2 rounded-xl transition-all">
                  Clear All
                </button>
              </div>
              
              <div className="grid gap-4">
                {messages.length === 0 && <p className="text-center py-20 text-stone-400">No messages found.</p>}
                {messages.map(m => (
                  <div key={m.id} className={cn(
                    ...
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex items-center gap-3">
                        <div className={cn("w-2.5 h-2.5 rounded-full", !m.read ? "bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.5)]" : "bg-stone-200")} />
                        <div>
                          <p className="font-bold">{m.username}</p>
                          <p className="text-xs text-stone-400 tracking-tight">{m.email}</p>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        {!m.read && (
                        ...
                        <button onClick={() => deleteDoc(doc(db, "contactMessages", m.id))}
                                className="p-2.5 rounded-xl bg-stone-50 text-stone-400 hover:bg-rose-600 hover:text-white transition-all">
                          <Trash2 size={16} />
                        </button>
                      </div>
                    </div>
                    <p className="text-stone-600 text-sm leading-relaxed bg-stone-50 p-5 rounded-2xl border border-stone-100 select-all">
                      {m.message}
                    </p>
                  </div>
                ))}
            ...
            <div className="max-w-2xl mx-auto">
              <div className="bg-white rounded-[3rem] border border-stone-200 shadow-sm p-10 space-y-8">
                <div className="text-center space-y-2">
                  <div className="w-16 h-16 bg-rose-50 text-rose-600 rounded-[1.5rem] flex items-center justify-center mx-auto mb-4">
                    <Bell size={32} />
                  </div>
                  <h3 className="text-2xl font-bold tracking-tight">Push Notification</h3>
                  <p className="text-stone-400 text-sm">Send a global alert to all registered users instantly.</p>
                </div>
                ...
                <form onSubmit={async (e: any) => {
                  ...
                  <input name="title" required placeholder="Notification Title" 
                         className="w-full px-6 py-4 rounded-2xl bg-stone-50 border border-stone-200 focus:ring-2 ring-rose-100 outline-none font-medium" />
                  <textarea name="body" rows={5} required placeholder="Write your message here..."
                            className="w-full px-6 py-4 rounded-2xl bg-stone-50 border border-stone-200 focus:ring-2 ring-rose-100 outline-none resize-none font-medium" />
                  <button className="w-full py-4 rounded-2xl bg-rose-600 text-white font-bold hover:bg-rose-700 shadow-lg shadow-rose-100 transition-all flex items-center justify-center gap-2">
          ...
          {/* Settings Tab */}
          {activeTab === "settings" && (
            <div className="max-w-3xl mx-auto bg-white rounded-[2.5rem] border border-stone-200 p-10 shadow-sm">
              ...
              <div className="space-y-10">
                <div className="flex items-center gap-8 p-6 bg-stone-50 rounded-[2rem] border border-dashed border-stone-200">
                   ...
                   <div className="space-y-2">
                     <p className="font-bold text-sm">Application Icon</p>
                     ...
                     }} className="text-xs text-stone-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-black file:text-white hover:file:bg-stone-800 cursor-pointer" />
                   </div>
                </div>

                <div className="space-y-4">
                  <label className="text-sm font-bold px-2">Support Inquiry Email</label>
                  <input 
                    value={settings.contactRecipientEmail || ""}
                    onChange={e => setSettings({ ...settings, contactRecipientEmail: e.target.value })}
                    className="w-full px-6 py-4 rounded-2xl bg-stone-50 border border-stone-200 focus:ring-2 ring-black/5 outline-none font-medium transition-all"
                ...
                <button 
                  ...
                  className="w-full py-4 rounded-2xl bg-black text-white font-bold hover:shadow-xl hover:-translate-y-0.5 transition-all"
                >
                  Save Changes
                </button>
              </div>
          ...
          {activeTab === "security" && (
            <div className="space-y-6">
              <h3 className="text-xl font-bold px-2">Password Reset Requests</h3>
              <div className="bg-white rounded-[2.5rem] border border-stone-200 overflow-hidden">
                {resetRequests.length === 0 && <p className="p-10 text-center text-stone-400">No active requests.</p>}
                {resetRequests.map(r => (
                  <div key={r.id} className="flex justify-between items-center p-6 border-b border-stone-50 last:border-none">
                    <div>
                    ...
                    {r.status !== "completed" ? (
                      <button 
                        ...
                        className="px-6 py-2 rounded-xl bg-stone-900 text-white text-sm font-bold hover:bg-black transition-all">
                        Mark Resolved
                      </button>
                    ) : (
