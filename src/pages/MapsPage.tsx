...
import { Loader2, Info, Plus, Clock } from 'lucide-react';
...
const MapAutoZoom = ({ events }: { events: any[] }) => {
  ...
  useEffect(() => {
    if (events.length > 0) {
      ...
      map.flyToBounds(bounds, { padding: [50, 50], maxZoom: 13, animate: true, duration: 2 });
    }
  }, [events, map]);
  return null;
};
                ...
                <Marker key={idx} position={[first.lat, first.lng]}>
                  <Popup className="custom-popup">
                    <div className="p-1 min-w-[150px]">
                      {group.map((event: any, eIdx: number) => (
                        <div key={event.id} className={eIdx !== 0 ? "mt-3 pt-3 border-t border-stone-100" : ""}>
                          <div className="flex items-center gap-2 mb-1">
                            <div className={`w-2 h-2 rounded-full ${event.source === 'pin' ? 'bg-blue-500' : 'bg-rose-500'}`} />
                            <h3 className="font-black text-stone-900 m-0 text-sm leading-tight">{event.title}</h3>
                          </div>
                          <p className="text-[10px] text-stone-400 font-bold m-0 uppercase tracking-tighter flex items-center gap-1">
                            <Clock size={10} /> {event.time} | {event.date}
                          </p>
                          <button 
                            onClick={() => navigate(`/?date=${event.date}&openEvent=${event.id}`)}
                            className="mt-2 w-full py-1.5 bg-stone-900 text-white text-[10px] font-black rounded-lg uppercase transition-colors hover:bg-black"
                          >
                            View in Calendar
                          </button>
                        </div>
