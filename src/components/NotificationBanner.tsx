...
export default function NotificationBanner() {
  ...
  /* ---------------------------------- */
  /* Decide if banner should appear     */
  /* ---------------------------------- */

  useEffect(() => {

    // المتصفح لا يدعم الاشعارات
    if (!("Notification" in window)) return;

    const dismissed = localStorage.getItem("notifications-dismissed");

    // إذا المستخدم رفض الإشعارات نهائياً
    if (Notification.permission === "denied") {
      setVisible(false);
      return;
    }

    // إذا الإشعارات مفعلة بالفعل
  ...
  return (
    <div className="fixed top-0 left-0 w-full z-50 flex justify-center px-4 pt-4">
      ...
      <div className="w-full max-w-4xl rounded-3xl shadow-xl backdrop-blur-xl bg-white/90 dark:bg-stone-900/90 border border-stone-200 dark:border-stone-800 p-6">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-stone-600 dark:text-stone-400">
              Stay updated with reminders, events and admin announcements.
            </p>
          </div>
